                    ...
                    <table class="min-w-full border border-gray-200">
                        <thead class="bg-gray-100">
                            <tr>
                                ...
                                <th class="px-4 py-2 text-left text-gray-700 border-b">Mode of Payment</th>
                                <th class="px-4 py-2 text-left text-gray-700 border-b">Total</th>
                                <th class="px-4 py-2 text-left text-gray-700 border-b">Paid</th>
                        ...
                        <tbody>
                            <tr class="border-b" v-for="(item, index) in filteredReservations" :key="index">
                                ...
                                <td class="px-4 py-2">{{ item.mode_of_payment }}</td>
                                <td class="px-4 py-2">₱{{ item.total_amount }}</td>
                                <td class="px-4 py-2">₱{{ item.total_payments === null ? 0 : item.total_payments }}</td>
                                <td class="px-4 py-2">₱{{ item.total_amount - item.total_payments }}</td>
                                <td class="px-4 py-2 flex space-x-2">
                                    <button @click="viewItem(item)" type="button" class="text-white bg-indigo-500 hover:bg-indigo-600 focus:ring-4 focus:outline-none focus:ring-indigo-300 font-medium rounded-lg text-sm px-3 py-2 text-center inline-flex items-center me-2">
                                    ...
                                    <button @click="payItem(item)" v-if="item.total_amount - item.total_payments > 0" type="button" class="text-white bg-yellow-500 hover:bg-yellow-600 focus:ring-4 focus:outline-none focus:ring-yellow-300 font-medium rounded-lg text-sm px-3 py-2 text-center inline-flex items-center me-2">
                                        <svg class="w-6 h-6 text-white-800 dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
            ...
            <div class="relative p-4 w-full max-w-2xl max-h-full">
                ...
                <div class="relative bg-white rounded-lg shadow-sm ">
                    ...
                    <div class="flex items-center p-4 md:p-5 border-t border-gray-200 rounded-b">
                        <button @click="save" data-modal-hide="default-modal" type="button" class="text-white bg-green-700 hover:bg-green-800 focus:ring-4 focus:outline-none focus:ring-green-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center ">Pay</button>
                ...
                <div class="relative bg-white rounded-lg shadow-sm ">
                    ...
                    <div class="p-4 md:p-5 border-t border-gray-200 rounded-b">
                        <button
                            ...
                            type="button"
                            class="w-full py-2.5 px-5 text-sm font-medium text-gray-900 focus:outline-none bg-blue rounded-lg border border-blue-200 hover:bg-blue-100 hover:text-blue-700 focus:z-10 focus:ring-4 focus:ring-blue-100">
                            Close
                        </button>
                    </div>
...
export default {
    ...
    methods: {
        ...
        OpenDeleteForm(item) {
            ...
            this.delete_form.amount = item.amount
            this.delete_form.active = true
        },

        CloseDeleteForm() {
            this.delete_form.id = ''
            this.delete_form.amount = ''
            this.delete_form.active = false
        },

        save() {
            this.form.post('/payment/store', {
                onSuccess: () => {
                    this.closeForm()
        ...
        update() {
            this.form.patch('/payment/update/'+this.form.id, {
                onSuccess: () => {
                    this.closeForm()
        ...
        deleteItem() {
            this.form.delete('/payment/destroy/'+this.delete_form.id, {
                onSuccess: () => {
