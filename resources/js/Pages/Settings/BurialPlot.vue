<template>
    <AuthenticatedLayout>
        <Head title="Settings / Burial Plot" />
        ...
        <div class="py-1">
            <div class="mx-auto sm:px-6 lg:px-8">
                ...
                <div class="p-6 text-gray-900 flex flex-col md:flex-row md:justify-between md:items-center space-y-4 md:space-y-0 ">
                    <h3 class="text-2xl font-bold ">Settings - Burial Plot</h3>
                ...
                <div class="px-6 pb-6 overflow-x-auto bg-white shadow-md rounded-lg mt-0">
                    <div v-if="is_created" id="alert-border-1" class="flex items-center p-4 mb-4 text-green-800 border-t-4 border-green-300 bg-green-50 " role="alert">
                        ...
                        <div class="ms-3 text-sm font-medium">
                            Burial Plot Created Successfully.
                        </div>
                    ...
                    <div v-if="is_updated" id="alert-border-1" class="flex items-center p-4 mb-4 text-yellow-800 border-t-4 border-yellow-300 bg-yellow-50 " role="alert">
                        ...
                        <div class="ms-3 text-sm font-medium">
                            Burial Plot Updated Successfully.
                        </div>
                    ...
                    <div v-if="is_deleted" id="alert-border-1" class="flex items-center p-4 mb-4 text-red-800 border-t-4 border-red-300 bg-red-50 " role="alert">
                        ...
                        <div class="ms-3 text-sm font-medium">
                            Burial Plot Deleted Successfully.
                        </div>
                    ...
                    <table class="min-w-full border border-gray-200">
                        <thead class="bg-gray-100">
                            <tr>
                                ...
                                <th class="px-4 py-2 text-left text-gray-700 border-b">Plot #</th>
                                <th class="px-4 py-2 text-left text-gray-700 border-b">Size</th>
                                <th class="px-4 py-2 text-left text-gray-700 border-b">Type</th>
                        ...
                        <tbody>
                            <tr class="border-b" v-for="(item, index) in filterBurialTypes" :key="index">
                                <td class="px-4 py-2">{{ index + 1 }}</td>
                                <td class="px-4 py-2">{{ item.plot_number }}</td>
                                <td class="px-4 py-2">{{ item.size }}</td>
                                <td class="px-4 py-2">{{ item.burial_type != null ? item.burial_type.name : '' }}</td>
                ...
                <div class="relative bg-white rounded-lg shadow-sm ">
                    ...
                    <div class="flex items-center justify-between p-4 md:p-5 border-b rounded-t border-gray-200">
                        <h3 class="text-xl font-semibold text-gray-900 ">
                            {{ form.is_editing ? 'Edit' : 'Create' }} Burial Plot
                        </h3>
                    ...
                    <div class="p-4 md:p-5 space-y-4">
                        <div>
                            ...
                            <input v-model="form.size" type="text" name="email" id="email" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 " required />
                        </div>
                        <div>
                            <label for="countries" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Burial Type</label>
                            <select v-model="form.burial_type_id" id="countries" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                                <option v-for="(item, index) in burial_types" :key="index" :value="item.id">{{item.name }}</option>
                            </select>
                        </div>
                    </div>
                    <!-- Modal footer -->
...
export default {
    props: ['burial_types', 'burial_plots', 'errors'],
    components: {
        AuthenticatedLayout,
        Link,
        Head
    ...
    computed: {
        filterBurialTypes() {
            if (!this.search) return this.burial_plots;

            return this.burial_plots.filter(t => {
    ...
    methods: {
        ...
        editItem(item) {
            ...
            this.form.plot_number = item.plot_number
            this.form.size = item.size
            this.form.burial_type_id = item.burial_type_id
        ...
        OpenDeleteForm(item) {
            ...
            this.delete_form.name = item.plot_number
        ...
        save() {
            this.form.post('/burial_plot/store', {
                onSuccess: () => {
                    this.closeForm()
        ...
        update() {
            this.form.patch('/burial_plot/update/'+this.form.id, {
                onSuccess: () => {
                    this.closeForm()
        ...
        deleteItem() {
            this.form.delete('/burial_plot/destroy/'+this.delete_form.id, {
                onSuccess: () => {
